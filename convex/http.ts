 ...
 * 3. If not cached:
 *    a. Convert mxc:// to file:// via Beeper's /v1/assets/download API
 *    b. Fetch image data from local media server (beeperimage.bywave.com.au tunnel)
 *    c. Store in Convex file storage
 *    d. Save cache record to database
 *    e. Return the image
 * 
 * Requirements:
 * - beeper-media-server running locally (port 47392)
 * - Tunnel from beeperimage.bywave.com.au → localhost:47392
 * 
 * Benefits:
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    const BEEPER_MEDIA_TOKEN = process.env.BEEPER_MEDIA_TOKEN;
    ...
    try {
      ...
      // Step 3: Fetch from media server (beeperimage.bywave.com.au)
      // This server has direct access to the local media files via tunnel
      const mediaServerUrl = `${BEEPER_MEDIA_SERVER}/${mediaId}?token=${BEEPER_MEDIA_TOKEN}`;
      console.log(`[Image Proxy] Fetching from media server: ${mediaServerUrl.substring(0, 80)}...`);
      
      const response = await fetch(mediaServerUrl);

      if (!response.ok) {
        console.error(`[Image Proxy] Media server returned ${response.status} for ${mediaId}`);
        return new Response('Cannot access image from media server', { status: 502 });
      }

      console.log(`[Image Proxy] ✅ Success from media server`);
