...
export const Route = createFileRoute('/_authenticated/todos/$documentId')({
  component: TodoDocumentPage,
})

function TodoDocumentPage() {
  ...
  const handleDeleteDocument = async () => {
    ...
    if (remainingDocs && remainingDocs.length > 0) {
      navigate({ to: '/todos/$documentId', params: { documentId: remainingDocs[0]._id } })
    } else {
      navigate({ to: '/todos' })
    }
  }

  // The sidebar is rendered by the parent layout, we just render the content area
